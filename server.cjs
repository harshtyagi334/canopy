...
var KNOWN_GEO_REGIONS = {
  ...
  "kutch": { lat: 23.242, lon: 69.6669, state: "Gujarat", district: "Kutch", desc: "Mundra Coastal Mangrove Biosphere Strip" },
  "raigarh": { lat: 21.8974, lon: 83.395, state: "Chhattisgarh", district: "Raigarh", desc: "Gharghoda Coal Belt Compensatory Plantation" },
...
app.get("/api/satellite/status", async (req, res) => {
  ...
  res.json({
    ...
    credentialsConfigured: {
      ...
      note: hasCopernicusKeys || hasSentinelHubKeys ? "Enterprise authenticated feed active." : "Running on public open-access Earth Observation STAC registry (no keys required for search & browse imagery)."
    }
  });
});
app.post("/api/satellite/geocode", (req, res) => {
...
app.post("/api/satellite/search", async (req, res) => {
  try {
    ...
    const endStr = dateRange?.end || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    ...
    if (rawScenes.length > 0) {
      formattedScenes = rawScenes.map((f) => {
    ...
    if (formattedScenes.length === 0) {
      ...
      formattedScenes = referenceDates.map((ref, idx) => ({
        ...
        bbox: queryBbox,
        centroid: [centerLat, centerLon],
        thumbnailUrl: idx === 4 ? "https://sentinel-cogs.s3.us-west-2.amazonaws.com/sentinel-s2-l2a-cogs/43/Q/EV/2024/3/S2A_43QEV_20240331_0_L2A/thumbnail.jpg" : "",
        visualUrl: "",
...
app.post("/api/satellite/process", (req, res) => {
  try {
    ...
    const executionLogs = [
      ...
      `[${(/* @__PURE__ */ new Date()).toISOString()}] [VERDICT] Net recovery deficit: ${(deficitRatio * 100).toFixed(1)}%. Risk score assigned: ${complianceRiskScore}/100.`
    ];
    res.json({
      success: true,
      ...
      telemetry: {
        ...
        recommendedAction: complianceRiskScore > 65 ? "Issue Form-IV Non-Compliance Notice under Forest Conservation Act 1980; summon Divisional Forest Officer for physical ground audit." : "Schedule routine 90-day orbital monitoring pass."
      },
      executionLogs
    });
  } catch (err) {
